...
const JOBS = [
  { id: "JB-001", contractor: "GoldFields Mining", route: "Johannesburg → Rustenburg", cargo: "Heavy machinery", status: "active", transporter: "Lucky Mokoena", truck: "GP 12-34 XY", value: 18500, commission: 2220, eta: "2h 14m", progress: 65 },
  ...
  { id: "JB-003", contractor: "BuildRight Corp", route: "Durban → Cape Town", cargo: "Steel beams", status: "completed", transporter: "Sipho Dlamini", truck: "KZN 55-67 AB", value: 32000, commission: 3840, eta: "Delivered", progress: 100 },
  { id: "JB-004", contractor: "Harmony Gold", route: "Welkom → Carletonville", cargo: "Mining equipment", status: "active", transporter: "Themba Nkosi", truck: "FS 88-12 CD", value: 24700, commission: 2964, eta: "4h 52m", progress: 30 },
  { id: "JB-005", contractor: "FreshPro Logistics", route: "Cape Town → George", cargo: "Perishables", status: "disputed", transporter: "Andre van Wyk", truck: "WC 44-21 EF", value: 7800, commission: 936, eta: "Delayed", progress: 80 },
];
...
const statusStyle = (status) => {
  ...
  return map[status] || { bg: "#1a1a1a", color: "#aaa", label: status.toUpperCase() };
};
...
function StatCard({ label, value, sub, accent }) {
  return (
    <div style={{
      ...
      <div style={{ color: "#666", fontSize: 10, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8, fontFamily: "'Courier New', monospace" }}>{label}</div>
      <div style={{ color: "#fff", fontSize: 28, fontWeight: 700, fontFamily: "'Bebas Neue', 'Impact', sans-serif", letterSpacing: 1 }}>{value}</div>
      {sub && <div style={{ color: accent || "#f97316", fontSize: 11, marginTop: 4, fontFamily: "'Courier New', monospace" }}>{sub}</div>}
    </div>
  );
}
...
function ProgressBar({ pct, color = "#f97316" }) {
  return (
    <div style={{ background: "#1a1a1a", borderRadius: 1, height: 4, width: "100%", overflow: "hidden" }}>
      <div style={{ width: `${pct}%`, height: "100%", background: pct === 100 ? "#3b82f6" : pct === 0 ? "#333" : color, transition: "width 0.6s ease" }} />
    </div>
  );
}

function Overview() {
  const totalRevenue = PAYMENTS.filter(p => p.status === "released").reduce((a, p) => a + p.commission, 0);
  const escrow = PAYMENTS.filter(p => p.status === "in-escrow").reduce((a, p) => a + p.amount, 0);
  const activeJobs = JOBS.filter(j => j.status === "active").length;
  const availableTransporters = TRANSPORTERS.filter(t => t.status === "available").length;

  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        <div style={{ color: "#666", fontSize: 11, letterSpacing: 3, fontFamily: "'Courier New', monospace", marginBottom: 4 }}>// PLATFORM OVERVIEW</div>
        <h2 style={{ color: "#fff", fontSize: 32, margin: 0, fontFamily: "'Bebas Neue', Impact, sans-serif", letterSpacing: 2 }}>COMMAND CENTER</h2>
      </div>

      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 32 }}>
        <StatCard label="Commission Earned" value={`R ${totalRevenue.toLocaleString()}`} sub="↑ 18% this month" accent="#22c55e" />
        <StatCard label="Funds in Escrow" value={`R ${escrow.toLocaleString()}`} sub="2 jobs in transit" accent="#eab308" />
        <StatCard label="Active Jobs" value={activeJobs} sub="Live on platform" accent="#f97316" />
        <StatCard label="Available Trucks" value={availableTransporters} sub="Ready to dispatch" accent="#3b82f6" />
        <StatCard label="Total Transporters" value={TRANSPORTERS.length} sub="4 verified" accent="#a855f7" />
      </div>

      {/* Recent activity */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: "#0f0f0f", border: "1px solid #2a2a2a", borderRadius: 2, padding: 20 }}>
          <div style={{ color: "#666", fontSize: 10, letterSpacing: 2, fontFamily: "'Courier New', monospace", marginBottom: 16 }}>// LIVE JOBS</div>
          {JOBS.filter(j => j.status === "active").map(job => (
            <div key={job.id} style={{ marginBottom: 16, paddingBottom: 16, borderBottom: "1px solid #1a1a1a" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <span style={{ color: "#f97316", fontSize: 12, fontFamily: "'Courier New', monospace" }}>{job.id}</span>
                <span style={{ color: "#22c55e", fontSize: 11, fontFamily: "'Courier New', monospace" }}>ETA {job.eta}</span>
              </div>
        ...
        <div style={{ background: "#0f0f0f", border: "1px solid #2a2a2a", borderRadius: 2, padding: 20 }}>
          <div style={{ color: "#666", fontSize: 10, letterSpacing: 2, fontFamily: "'Courier New', monospace", marginBottom: 16 }}>// ALERTS & FLAGS</div>
          {[
            ...
            <div key={i} style={{ display: "flex", gap: 12, marginBottom: 14, alignItems: "flex-start" }}>
              ...
              <div>
                ...
                <div style={{ color: "#555", fontSize: 10, fontFamily: "'Courier New', monospace", marginTop: 2 }}>{a.time}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ color: "#666", fontSize: 11, letterSpacing: 3, fontFamily: "'Courier New', monospace", marginBottom: 4 }}>// JOB MANAGEMENT</div>
        <h2 style={{ color: "#fff", fontSize: 32, margin: 0, fontFamily: "'Bebas Neue', Impact, sans-serif", letterSpacing: 2 }}>JOB BOARD</h2>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {filtered.map(job => (
          <div key={job.id} style={{ background: "#0f0f0f", border: "1px solid #1f1f1f", borderRadius: 2, padding: "16px 20px" }}>
...
function Fleet() {
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ color: "#666", fontSize: 11, letterSpacing: 3, fontFamily: "'Courier New', monospace", marginBottom: 4 }}>// VEHICLE MANAGEMENT</div>
        <h2 style={{ color: "#fff", fontSize: 32, margin: 0, fontFamily: "'Bebas Neue', Impact, sans-serif", letterSpacing: 2 }}>FLEET TRACKER</h2>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
        {[
          ...
          <div key={truck.reg} style={{ background: "#0f0f0f", border: `1px solid ${truck.status === "flagged" ? "#3d0d0d" : "#1f1f1f"}`, borderRadius: 2, padding: 16 }}>
...
function Transporters() {
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ color: "#666", fontSize: 11, letterSpacing: 3, fontFamily: "'Courier New', monospace", marginBottom: 4 }}>// TRANSPORTER REGISTRY</div>
        <h2 style={{ color: "#fff", fontSize: 32, margin: 0, fontFamily: "'Bebas Neue', Impact, sans-serif", letterSpacing: 2 }}>TRANSPORTERS</h2>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {TRANSPORTERS.map(t => (
          <div key={t.id} style={{ background: "#0f0f0f", border: "1px solid #1f1f1f", borderRadius: 2, padding: "16px 20px", display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
            <div style={{
            ...
            <button style={{ padding: "6px 16px", background: "transparent", color: "#f97316", border: "1px solid #f97316", borderRadius: 2, cursor: "pointer", fontSize: 11, letterSpacing: 1, fontFamily: "'Courier New', monospace" }}>
              VIEW PROFILE
            </button>
          </div>
        ))}
...
function Payments() {
  const totalCommission = PAYMENTS.reduce((a, p) => a + p.commission, 0);
  const released = PAYMENTS.filter(p => p.status === "released").reduce((a, p) => a + p.commission, 0);
  const escrow = PAYMENTS.filter(p => p.status === "in-escrow").reduce((a, p) => a + p.amount, 0);

  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ color: "#666", fontSize: 11, letterSpacing: 3, fontFamily: "'Courier New', monospace", marginBottom: 4 }}>// FINANCIAL OVERVIEW</div>
        <h2 style={{ color: "#fff", fontSize: 32, margin: 0, fontFamily: "'Bebas Neue', Impact, sans-serif", letterSpacing: 2 }}>PAYMENTS</h2>
      </div>

      <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
        <StatCard label="Total Commission" value={`R ${totalCommission.toLocaleString()}`} sub="All time" accent="#22c55e" />
        <StatCard label="Released" value={`R ${released.toLocaleString()}`} sub="Cleared to bank" accent="#3b82f6" />
        <StatCard label="In Escrow" value={`R ${escrow.toLocaleString()}`} sub="Pending delivery" accent="#eab308" />
        <StatCard label="Disputed" value="R 936" sub="Needs resolution" accent="#ef4444" />
      </div>
...
function Documents() {
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ color: "#666", fontSize: 11, letterSpacing: 3, fontFamily: "'Courier New', monospace", marginBottom: 4 }}>// DOCUMENT VAULT</div>
        <h2 style={{ color: "#fff", fontSize: 32, margin: 0, fontFamily: "'Bebas Neue', Impact, sans-serif", letterSpacing: 2 }}>DOCUMENTS</h2>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 10 }}>
        {docs.map((doc, i) => {
          const statusMap = { verified: "#22c55e", expired: "#ef4444", pending: "#eab308", signed: "#3b82f6" };
          return (
            <div key={i} style={{ background: "#0f0f0f", border: "1px solid #1f1f1f", borderRadius: 2, padding: 16, display: "flex", gap: 14, alignItems: "center" }}>
              <div style={{ width: 40, height: 48, background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", color: "#f97316", fontSize: 12, fontFamily: "'Courier New', monospace", flexShrink: 0 }}>
                {doc.type}
              </div>
              <div style={{ flex: 1, overflow: "hidden" }}>
                ...
                <div style={{ color: statusMap[doc.status], fontSize: 10, letterSpacing: 1, fontFamily: "'Courier New', monospace", marginTop: 4, textTransform: "uppercase" }}>
...
export default function TruckerDashboard() {
  ...
  const pages = { overview: Overview, jobs: JobBoard, fleet: Fleet, transporters: Transporters, payments: Payments, documents: Documents };
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes pulse { 0%,100% { box-shadow: 0 0 0 4px #22c55e33, 0 0 0 8px #22c55e11; } 50% { box-shadow: 0 0 0 8px #22c55e22, 0 0 0 16px #22c55e08; } }
      `}</style>
      ...
      <div style={{ width: 220, background: "#050505", borderRight: "1px solid #141414", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <nav style={{ flex: 1, padding: "16px 0" }}>
          {NAV_ITEMS.map(item => (
            <button key={item.id} onClick={() => setActive(item.id)} style={{
              ...
              border: "none", borderLeft: active === item.id ? "3px solid #f97316" : "3px solid transparent",
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid #141414" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#f97316", display: "flex", alignItems: "center", justifyContent: "center", color: "#000", fontWeight: 700, fontSize: 14 }}>A</div>
            <div>
      ...
      <div style={{ flex: 1, overflow: "auto" }}>
        {/* Topbar */}
        <div style={{ padding: "16px 32px", borderBottom: "1px solid #141414", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#050505", position: "sticky", top: 0, zIndex: 10 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: "50%", width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
